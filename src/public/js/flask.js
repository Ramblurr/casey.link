...
function createPopLines(flask, x, y, size) {
    ...
    const lines = [];
    const lineLength = size * 3; // INCREASED from size*2 to size*3

    // Create 4 lines in X pattern
    const angles = [45, 135, 225, 315]; // Angles in degrees
    ...
    angles.forEach((angle) => {
        ...
        line.setAttribute("stroke-width", "2"); // INCREASED from 1 to 2
...
function updateBubblePopAnimation(bubble, lines, x, y, size, progress) {
    ...
    // Maximum line length matches what we use in createPopLines (size * 3)
    ...
    if (progress < 0.2) {
        ...
        // Update lines - start to appear
        if (lines) {
            lines.forEach((line, i) => {
                const angle = ([45, 135, 225, 315][i] * Math.PI) / 180;
                const lineLength = size * 1.5 * phase1Progress; // Line grows from 0 to size*1.5
...
function initializeBubbles(flask) {
    ...
    // Get all existing bubbles from this specific flask
    ...
    existingBubbles.forEach((bubble) => {
        ...
        // Create state object for this bubble
...
function createBubbleState(flask, bubble, x, y, size, isDynamic) {
    ...
    const sizeFactor = size / 4; // Normalize around size = 4

    // Store all bubble data in the map
...
function animateBubble(flask, bubble) {
    ...
    const animate = () => {
        ...
        if (state.isPopping) {
            // Animate popping
            const progress =
                (Date.now() - state.popStartTime) /
                BUBBLE_POP_ANIMATION_DURATION;
        ...
        if (flaskData.isAnimating || state.hasEscaped || state.isPopping) {
            flaskData.bubbleAnimations.set(
                bubble,
...
function stopAnimation(flask, mode = "freeze") {
    ...
    flaskData.bubbleState.forEach((state, bubble) => {
        ...
        // For bubbles outside the flask, we don't do anything - they'll continue floating
    });
}
...
function initFlaskAnimations() {
    // Find all flask SVGs with the ol-logo-flask class
...
// Store original bubble configurations for each flask
const originalBubbles = new Map();
...
/**
 * Stops ALL animation for a specific flask including escaped bubbles
 * @param {SVGElement|string} flaskElement - Flask SVG element or selector
 */
function stopAllFlaskAnimation(flaskElement) {
    // Handle both element and selector string
    const flask =
        typeof flaskElement === "string"
            ? document.querySelector(flaskElement)
    ...
    // Cancel ALL animation frames, including escaped bubbles
    flaskData.bubbleAnimations.forEach((frameId, bubble) => {
        cancelAnimationFrame(frameId);
    });

    flaskData.bubbleAnimations.clear();
}

/**
 * Stops animation for bubbles inside the flask but lets escaped bubbles continue
 * @param {SVGElement|string} flaskElement - Flask SVG element or selector
 */
function stopFlaskAnimation(flaskElement) {
    // Handle both element and selector string
    const flask =
        typeof flaskElement === "string"
            ? document.querySelector(flaskElement)
    ...
    // Use the existing partial stop function
    stopAnimation(flask, "freeze");
}

/**
 * Resets a flask to its original state
 * @param {SVGElement|string} flaskElement - Flask SVG element or selector
 */
function resetFlaskAnimation(flaskElement) {
    // Handle both element and selector string
    const flask =
        typeof flaskElement === "string"
            ? document.querySelector(flaskElement)
    ...
    stopAllFlaskAnimation(flask);

    // Remove all dynamic bubbles
    flaskData.bubbleState.forEach((state, bubble) => {
        if (state.isDynamic) {
            if (
                flaskData.bubblesContainer &&
            ...
            // For original bubbles, reset visibility
            bubble.style.display = "";
        }
    });

    // Clear state
    flaskData.bubbleState = new Map();

    // If we have stored original bubbles, restore them
    if (originalBubbles.has(flask)) {
        const bubbleContainer = flaskData.bubblesContainer;

        // Clear existing bubbles from the container
        while (bubbleContainer.firstChild) {
            bubbleContainer.removeChild(bubbleContainer.firstChild);
        }

        // Restore original bubbles
        originalBubbles.get(flask).forEach((config) => {
            const newBubble = config.element.cloneNode(true);
            bubbleContainer.appendChild(newBubble);

            // Reset to original shape
            updateBubbleShape(newBubble, config.x, config.y, config.size, 0, 0);
...
// Initialize all flasks when the DOM is loaded
